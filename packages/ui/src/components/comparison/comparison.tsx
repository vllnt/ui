    ...
    <div className="my-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className={cn('rounded-lg border', beforeConfig.className)}>
          <div
            className={cn('px-4 py-2 rounded-t-lg font-medium text-sm', beforeConfig.headerClass)}
          >
            {before.title}
          </div>
          <ul className="p-4 space-y-2">
            {before.items.map((item, index) => (
              <li className="flex items-start gap-2 text-sm" key={index}>
                <BeforeIcon
                  className={cn('h-4 w-4 mt-0.5 flex-shrink-0', beforeConfig.iconClass)}
                />
                <span>{item}</span>
              </li>
            ))}
        ...
        <div className={cn('rounded-lg border', afterConfig.className)}>
          <div
            className={cn('px-4 py-2 rounded-t-lg font-medium text-sm', afterConfig.headerClass)}
          >
            {after.title}
          </div>
          <ul className="p-4 space-y-2">
            {after.items.map((item, index) => (
              <li className="flex items-start gap-2 text-sm" key={index}>
                <AfterIcon className={cn('h-4 w-4 mt-0.5 flex-shrink-0', afterConfig.iconClass)} />
                <span>{item}</span>
              </li>
            ))}
          </ul>
