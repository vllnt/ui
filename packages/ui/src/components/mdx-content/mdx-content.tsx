...
const MDXComponents: Components = {
  ...
  p: ({ children, ...props }: React.ComponentProps<'p'>) => (
    <p className="mb-6 leading-relaxed text-muted-foreground text-sm max-w-none" {...props}>
      {children}
    </p>
  ),
...
function removeImportStatements(content: string, componentNames: string[]): string {
  let processed = content.replaceAll(/^import\s+.*CodeBlock.*from.*$/gm, '')
  componentNames.forEach((name) => {
    processed = processed.replaceAll(new RegExp(`^import\\s+.*${name}.*from.*$`, 'gm'), '')
  })
  return processed
}
...
export async function MDXContent({ components = {}, content, enableMDX = true }: MDXContentProps) {
  const componentNames = Object.keys(components)
  const processedContent = removeImportStatements(content, componentNames)
  const contentWithoutCodeBlocks = processedContent.replaceAll(/```[\S\s]*?```/g, '')
