...
import type { ComponentCategory, Registry, RegistryComponent } from '@/types/registry'
...
function groupComponentsByCategory(
  ...
  const grouped = items.reduce((accumulator, item) => {
    ...
    accumulator.set(category, [...existing, { name: item.name, title: item.title }])
    return accumulator
...
export function getSidebarSections(activeCategory?: ComponentCategory) {
  return [
    ...
    ...groupedComponents.map((group) => ({
      collapsible: true,
      defaultOpen: activeCategory ? group.category === activeCategory : group.category === 'core',
...
export function getCategoryForComponent(componentName: string): ComponentCategory | undefined {
