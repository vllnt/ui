...
export async function GET(_request: Request, routeParameters: RouteParameters) {
  try {
    ...
    const component = registry.items.find(
      (item): item is RegistryComponent => item.name === name && item.type === 'registry:component',
    )
    ...
    const files = await Promise.all(component.files.map((file) => readComponentFile(file.path)))
