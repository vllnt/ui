...
- `platforms`: non-empty renderer list, using `web`, `native`, or both.
- `native`: package, channel, release-status, and parity metadata when `native` is supported.
- `files`: source files installed by the web CLI.
...
Agents should prefer `/r/registry.json` for discovery and `/r/<name>.json` for implementation details. Check `platforms` and `native.availability` before selecting an installation path. Web descriptors install through shadcn. Native pilot entries currently expose source metadata only; `@vllnt/ui-native@canary` is planned but not published. Use `/llms-full.txt` when a single text context is easier than fetching many JSON endpoints.
