...
- **React Native:** the renderer is currently source-only and has no working npm installation. Review the [React Native guide](/docs/native?platform=native) and [native catalog](/components?platform=native); do not run the planned canary command until the native manifest reports package availability.
