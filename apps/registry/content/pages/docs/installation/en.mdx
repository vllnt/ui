...
Choose the renderer that matches your application before installing anything.

- **Web:** use the published `@vllnt/ui` package or copy individual components from the shadcn-compatible registry. Continue below for the web setup.
- **React Native · Experimental:** the renderer is currently source-only and has no working npm installation. Review the [React Native guide](/docs/native?platform=native) and [native catalog](/components?platform=native); do not run the planned canary command until the native manifest reports package availability.
