...
- PR body covers: linked issue (GitHub keywords `close` / `closes` / `closed`, `fix` / `fixes` / `fixed`, `resolve` / `resolves` / `resolved`, or repo phrases `Part of` / `Related to` — all case-insensitive, optional colon — followed by `#123` or `owner/repo#123`), summary (why), evidence (CI run IDs, screenshots, numbers), test plan.
