...
**SYNC 2026-07-13 — GSC-validated path additions** (90d, `sc-domain:ui.vllnt.ai`): fold into `.4`/`.5`/`.6`/`.7`. Highest-impact **capture** paths the data proves, none of which exist today: `/alternatives/radix` (~20 impr across "radix dialog/slider/popover/scroll-area/button…", no page) + `/alternatives/mui` (cross-framework "material design X" / "angular X"); a `/react/{component}` framework-qualified namespace seeded from proven clusters — `react callout` (~60 impr, comp exists but buried pos 80+), `react floating action button` (~40 impr across react/angular/js variants, comp exists, pos 58–95); un-bury `callout` + `floating-action-button` in the `.6` uniqueness pass. **Create** paths (your AI pillar): `/guides/ai-ui` reads as `.7` flagship-guide territory — but GSC shows **zero** demand for "AI UI development", so it is a recall/GEO bet (compounding, not near-term impressions); AI *component* pages already rank pos 5–7 (`ai-chat-input`, `ai-source-citation`), so the pillar should funnel to `/families/ai` + those.
